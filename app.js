...
async function fetchAndRender(lat, lon) {
  const url = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current=uv_index,cloud_cover,temperature_2m,precipitation,is_day&hourly=uv_index,cloud_cover&daily=uv_index_max,cloud_cover_mean,temperature_2m_max&temperature_unit=fahrenheit&timezone=auto`;
  ...
  if (rating === 1) {
    info.desc = getRatingOneRoast(data.current.is_day, data.current.precipitation);
  }
