...
const RATING_INFO = {
  1: {
    label: "Not happening",
    color: "var(--bad)",
    desc: "UV is essentially flat right now."
  },
  2: {
...
// Special-case description for rating 1 (no real UV) — roasts based on
// whether it's night or just raining/heavily overcast.
function getRatingOneRoast(isDay, precipitation) {
  const raining = precipitation && precipitation > 0;
  if (!isDay) {
    return "u can not tan in ts idk what u thinking (it's literally night) u know 💀";
  }
  if (raining) {
    return "u can not tan in ts idk what u thinking (it's raining, stupid) u know 💀";
  }
  return "u can not tan in ts idk what u thinking (UV is basically zero) u know 💀";
}
